...
export class CodeManager implements vscode.Disposable {
  ...
  private initialize(): void {
    ...
    if (
      (this._config.get<boolean>("fileDirectoryAsCwd") ||
        !this._workspaceFolder) &&
      this._document &&
      !this._document.isUntitled
    ) {
    ...
    if (
      ...
      let text =
        ...
        ignoreSelection
          ...
          : this._document.getText(selection)
      ...
      const folder = this._document.isUntitled
        ? this._cwd
        : dirname(this._document.fileName)
  ...
  private getExecutor(languageId: string, fileExtension: string): string {
    this._languageId =
      languageId === null ? this._document.languageId : languageId
  ...
  private async getFinalCommandToRunCodeFile(
    ...
    return cmd !== executor
      ? cmd
      : executor + (appendFile ? " " + this.quoteFileName(this._codeFile) : "")
  }
  ...
  private changeFilePathForBashOnWindows(command: string): string {
    if (os.platform() === "win32") {
      ...
      if (windowsShell && terminalRoot) {
        ...
        command = command
          .replace(/([A-Za-z]):\\/g, this.replacer)
          .replace(/\\/g, "/")
      }
    }
    return command
